...
sub t_09_duped_primary_key : Tests {
    ...
    like $@, qr/(?:columns key1, key2, key3 are not unique|not unique columns|Duplicate entry 'a001-b001-c001' for key 'PRIMARY'|duplicate key value violates unique constraint|Duplicate entry)/;
}

sub t_09_duped_unique : Tests {
    eval { mock->set( multi_unique => { unq1 => 'a001', unq2 => 'b001', unq3 => 'c001' } ) };
    like $@, qr/(?:columns unq1, unq2, unq3|not unique columns|Duplicate entry 'a001-b001-c001' for key 'unq'|duplicate key value violates unique constraint|Duplicate entry)/;
}
