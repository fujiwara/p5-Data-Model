...
sub t_001_insert : Tests {
    ok(mock->set( tusr => 'id1', { name => 'name1' } ));
    ok(mock->set( tusr => 'id2', { name => 'name2' } ));
    ok(mock->set( tusr => 'id3', { name => 'name3' } ));
    ok(mock->set( tusr => 'id4', { name => 'name4' } ));
    ok(mock->set( tusr => 'id5', { name => 'name5' } ));
    ok(mock->set( tusr => 'id6', { name => 'name6' } ));
    ok(mock->set( tusr => 'id7', { name => 'name7' } ));
    ok(mock->set( tusr => 'id8', { name => 'name8' } ));
}

sub t_002_lookup_delete_lookup : Tests {
    my $get = mock->lookup( tusr => 'id3' );
    ok($get, 'lookup ok');
    is $get->id, 'id3', 'id ok';
    ...
    ok(!mock->lookup( tusr => 'id3' ), 'id3 is deleted');;
}

sub t_003_lookup_update_lookup : Tests {
    my $get = mock->lookup( tusr => 'id5' );
    ok($get, 'lookup ok');
    is $get->id, 'id5', 'id ok';
...
sub t_006_lookup_replace_lookup : Tests {
    my $get = mock->lookup( tusr => 'id8' );
    ok($get, 'lookup ok');
    is $get->id, 'id8', 'id ok';
    is $get->name, 'name8', 'name ok';

    ok(mock->replace( tusr => 'id8' => { name => "replace" } ), 'replace ok');

    $get = mock->lookup( tusr => 'id8' );
    ok($get, 'lookup ok');
    is $get->id, 'id8', 'id ok';
...
sub t_100_driver_change : Tests(1) {
    my $driver = mock->get_driver('tusr');
    return ok(1) unless $driver->{fallback};
    mock->set_driver('tusr',  $driver->{fallback});
    ok(1);
}

sub t_101_check_fallback_driver : Tests {
    my @get = mock->lookup_multi(
        tusr => [qw/ id1 id2 id3 id4 id5 id6 id7 id8 /]
    );
